...
class AnswerSearcher:
    ...
    def make_year_status_ans(self, answer: Answer, chain: TranslationChain, result: Result):
        ...
        answer.add_answer('{result["year"][0]}年，{data[0].info}')
    ...
    def make_catalog_status_ans(self, answer: Answer, builder: AnswerBuilder,
        ...
        for item, name in builder.product_data_with_name(
                result['catalog'],
                if_is_none=lambda _, na: '并没有关于{result["year"][0]}年{na.subject()}的描述'
        ):
    ...
    def make_exist_catalog_ans(self, answer: Answer, chain: TranslationChain, result: Result):
        data = self._search_direct(chain)
        if not all(data):
            answer.add_answer('无{result["year"][0]}年的记录。')
        else:
            answer.add_answer('{result["year"][0]}年目录包括: ' + '，'.join([item.name for item in data[0]]))
    ...
    def make_index_value_ans(self, answer: Answer, builder: AnswerBuilder,
        ...
        builder.feed_data(data)
        for item, name in builder.product_data_with_name(
                result['index'], if_is_none=lambda _, na: '无{na.subject()}数据记录'
        ):
            answer.add_answer('{name.subject()}为{item.val()}')
    ...
    def make_index_compose_ans(self, answer: Answer, builder: AnswerBuilder,
        ...
        for total, (item, name) in zip(
            ...
            if not item:
                answer.add_answer('指标“{name.name}”没有任何组成')
                continue
            indexes, areas = [], []
            for n in item:
            ...
            for k, v in zip(indexes + areas, data1 + data2):
                ...
                try:
                    ...
                    answer.add_answer('{name.name}中{k}的记录非数值类型，无法比较')
                    return
            # make other
            ...
            sub_titles.append('{name.subject()}为{total.val()}，其构成分为：')
        # paint
        for pie in self.painter.paint_pie(collect, units,
                                          title=result.raw_question, sub_titles=sub_titles):
            answer.save_chart(pie)
        answer.add_answer('该问题的回答已渲染为图像，详见：{CHART_RENDER_DIR}/{result.raw_question}.html')
    ...
    def make_indexes_m_or_n_compare_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        for (x, y), (n1, n2) in builder.product_data_with_binary(
                result['index'],
                if_x_is_none=lambda _1, _2, na: '无{na[0].subject()}数据记录，无法比较',
                if_y_is_none=lambda _1, _2, na: '无{na[1].subject()}数据记录，无法比较'
        ):
            # 单位检查
            ux, uy = x.unit or '无', y.unit or '无'
            if builder.add_if_is_equal_or_not(ux, uy,
                                              no='{n1.subject()}的单位（{ux}）与{n2.subject()}的单位（{uy}）不同，无法比较'):
                answer.begin_sub_answers()
                answer.add_sub_answers('{n1.subject()}为{x.val()}，{n2.subject()}为{y.val()}')
                res1 = builder.binary_calculation(x.value, y.value, operator)
                if builder.add_if_is_not_none(res1,
                                              no='{n1.subject()}或{n2.subject()}非数值类型，无法比较'):
                    if qt == 'indexes_m_compare':
                        answer.add_sub_answers('前者是后者的{res1}倍')
                    else:
                        answer.add_sub_answers('前者比后者{sign(res1)}{abs(res1)}{ux}')
                if qt == 'indexes_m_compare':
                    res2 = builder.binary_calculation(y.value, x.value, truediv)
                    if builder.add_if_is_not_none(res2,
                                                  no='{n1.subject()}或{n2.subject()}非数值类型，无法比较'):
                        answer.add_sub_answers('后者是前者的{res2}倍')
    ...
    def make_indexes_g_compare_ans(self, answer: Answer, builder: AnswerBuilder,
        ...
        for item, name in builder.product_data_with_name(result['index']):
            ...
            if builder.binary_decision(
                    x, y,
                    not_x='无{result["year"][0]}年关于{name.subject()}的数据',
                    not_y='无{result["year"][0]}前一年关于{name.subject()}的数据'
            ):
                res = builder.growth_calculation(y.value, x.value)
                if builder.add_if_is_not_none(
                        res, to_sub=False,
                        no='{result["year"][0]}年{name.subject()}的记录非数值类型，无法计算'
                ):
                    answer.add_answer('{result["year"][0]}年的{name.subject()}为{y.val()}，'
                                      '其去年的为{x.val()}，同比{sign(res, ("降低", "增长"))}{abs(res)}%')
    ...
    def make_area_value_ans(self, answer: Answer, builder: AnswerBuilder,
        ...
        for item, name in builder.product_data_with_name(
                ...
                if_is_none=lambda _, na: '{na.subject()}无数据记录'
        ):
            name.repr = item.repr
            answer.add_answer('{name.subject()}为{item.value}{item.unit}')
    ...
    def make_index_or_area_overall_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        for x, y, f, n in builder.product_data_with_feed(
                *gen,
                if_x_is_none=lambda _1, _2, _3, na: '无{na.subject()}的数据记录，无法比较',
                if_y_is_none=lambda _1, _2, _3, na: '无{na.subject()}的父级{tag}数据记录，无法比较'
        ):
            f.life_check(result['year'][0])
            if not f:
                answer.add_answer('无{n.subject()}父级{tag}数据记录，无法比较')
                return
            answer.begin_sub_answers()
            ...
            answer.add_sub_answers('{n.subject()}为{x.val()}')
            answer.add_sub_answers('其父级{tag}{f.subject()}为{y[0].val()}')
            if unit_x != unit_y:
                answer.add_sub_answers('两者单位不同，无法比较')
                answer.end_sub_answers()
                return
            res1 = builder.binary_calculation(x.value, y[0].value, truediv, percentage=True)
            if builder.add_if_is_not_none(res1, no='无效的{n.subject()}值类型，无法比较'):
                answer.add_sub_answers('前者占后者的{res1}%')
            res2 = builder.binary_calculation(y[0].value, x.value, truediv)
            if builder.add_if_is_not_none(res2, no='无效的{n.subject()}值类型，无法比较'):
                answer.add_sub_answers('后者是前者的{res2}倍')
            answer.end_sub_answers()
    ...
    def make_index_or_area_2_overall_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        for x, y, f, n in builder.product_data_with_feed(
                *gen,
                if_x_is_none=lambda _1, _2, _3, na: '无{years}这几年{na.subject()}的数据记录，无法比较',
                if_y_is_none=lambda _1, _2, _3, na: '无{years}这几年{na.subject()}的父级数据记录，无法比较'
        ):
            temp = []  # 记录两次计算的结果值
            for i, year in enumerate(result["year"]):
                f.life_check(year)
                if not f:
                    answer.add_answer('无{year}年{n.subject()}的父级数据记录，无法比较')
                    continue
                unit_x, unit_y = x[i].unit, y[i].unit
                answer.begin_sub_answers()
                n.repr = x[i].repr
                answer.add_sub_answers('{year}年{n.subject()}为{x[i].value}{unit_x}')
                answer.add_sub_answers('其总体{f.name}{y[i].repr}为{y[i].value}{unit_y}')
                if unit_x != unit_y:
                    answer.add_sub_answers('两者单位不同，无法比较')
                    answer.end_sub_answers()
                    continue
                res = builder.binary_calculation(x[i].value, y[i].value, truediv, percentage=True)
                if builder.add_if_is_not_none(res, no='无效的{n}值类型，无法比较'):
                    answer.add_sub_answers('约占总体的{res}%')
                    temp.append(res)
                answer.end_sub_answers()
            if len(temp) == 2:
                num = round(temp[0] - temp[1], 2)
                answer.add_answer('前者相比后者{sign(num, ("降低", "提高"))}{abs(num)}%')
    ...
    def make_indexes_or_areas_2m_or_2n_compare_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        for item, name in builder.product_data_with_name(*gen):
            ...
            if builder.binary_decision(
                    x, y,
                    not_x='无关于{result["year"][0]}年的{name.subject()}的记录',
                    not_y='无关于{result["year"][1]}年的{name.subject()}的记录'
            ):
                answer.begin_sub_answers()
                res = builder.binary_calculation(x.value, y.value, operator)
                if builder.add_if_is_not_none(res, no='{name.subject()}的记录为无效的值类型，无法比较'):
                    answer.add_sub_answers('{result["year"][0]}年的{name.subject()}为{x.val()}')
                    answer.add_sub_answers('{result["year"][1]}年的{name.subject()}为{y.val()}')
                    if qt in ('areas_2m_compare', 'indexes_2m_compare'):
                        ux, uy = x.unit, y.unit
                        # 单位为%的数值不支持此类型比较
                        if ux == uy == '%':
                            answer.add_sub_answers('它们单位为‘%’，不支持此类型的比较')
                        else:
                            answer.add_sub_answers('前者是后者的{res}倍')
                    else:
                        answer.add_sub_answers('前者比后者{sign(res, ("减少", "增加"))}{abs(res)}{x.unit}')
                answer.end_sub_answers()
        ...
        for item, name in builder.product_data_with_name(result['area'], result['index']):
            ...
            if builder.binary_decision(
                ...
                if builder.add_if_is_not_none(
                    ...
                    answer.add_answer('{result["year"][0]}年的{name.subject()}为{y.val()}，'
                                      '其去年的为{x.val()}，同比{sign(res, ("减少", "增长"))}{abs(res)}%')
    ...
    def make_catalog_or_index_change_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        if builder.add_if_is_equal_or_not(
                n1, 0, equal=False,
                no='{result["year"][1]}年与{result["year"][0]}年的{tag_name}相同'
        ):
            answer.add_answer('{result["year"][1]}年与{result["year"][0]}年相比，未统计{n1}个{tag_name}：' + '、'.join(diff1))
        if builder.add_if_is_equal_or_not(
                n2, 0, equal=False,
                no='{result["year"][0]}年与{result["year"][1]}年的{tag_name}相同'
        ):
            answer.add_answer('{result["year"][0]}年与{result["year"][1]}年相比，未统计{n2}个{tag_name}：' + '、'.join(diff2))
    ...
    def make_catalogs_or_indexes_change_ans(self, qt: str, answer: Answer, chain: TranslationChain, result: Result):
        ...
        line = self.painter.paint_line(result['year'], '{tag}个数', y, result.raw_question)
    ...
    def make_indexes_or_areas_trend_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        for item, name in builder.product_data_with_name(*gen):
            ...
            if builder.add_if_is_equal_or_not(sum(ys), 0, equal=False,
                                              no='指标“{name.subject()}”无任何值记录，无法比较'):
                for unit in units:
                    tmp = []
    ...
    def make_indexes_or_areas_overall_trend_ans(self, qt: str, answer: Answer, builder: AnswerBuilder,
        ...
        for x, y, f, n in builder.product_data_with_feed(
                *gen,
                if_x_is_none=lambda _1, _2, _3, na: '无关于”{na.subject()}“的记录',
                if_y_is_none=lambda _1, _2, _3, na: '无关于”{na.subject()}“的父级记录'
        ):
            xs = builder.group_mapping_to_float(x)
            if not builder.add_if_is_not_none(xs, to_sub=False,
                                              no='{n.subject()}的记录非数值类型，无法比较'):
                return
            ...
            if not builder.add_if_is_not_none(ys, to_sub=False,
                                              no='{n.subject()}的父级记录（{parent}）非数值类型，无法比较'):
                return
    ...
    def make_begin_stats_ans(self, answer: Answer, builder: AnswerBuilder,
        ...
        for item, name in builder.product_data_with_name(result['index']):
            ...
            answer.add_answer('指标“{name.name}”最早于{min(years)}年开始统计')
